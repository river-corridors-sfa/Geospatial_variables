...
# Filter the sites to only have SSS sites based on sampling metadata from the DP

metadata = read.csv('SSS_Data_Package/SSS_Field_Metadata.csv')%>% dplyr::select('Site_ID')

sss = merge(sites,metadata, by = 'Site_ID')

sites = sss %>% dplyr::select(site = 'Site_ID', comid = 'COMID')
```
